...
# =========================================================
# Implementaciones relacionadas al algoritmo aváro
# =========================================================
...
def greedy(start_node, goal_node):
    ...
    print()
    print("Actual: -")
    print(f"Abiertos: [{start_node.name}_{start_node.name}: {straight_line_distance[start_node.name, goal_node.name]}]")
    print("Cerrados: []")

    new_tuple = (start_node, start_node, straight_line_distance[start_node.name, goal_node.name])
    ...
    while current[0] is not goal_node:
        ...
        for neighbor in current[0].neighbors:
            ...
            if not node_is_in_closed(neighbor, closed):
                new_tuple = (neighbor, current[0], straight_line_distance[neighbor.name, goal_node.name])
                open.append(new_tuple)
...
def clean_open_from_duplicates(open):
    ...
    for tuple in open:
        if tuple[0] not in nodes_accounted_for: # Si el nodo no ha sido tomado en cuenta se mete a la lista
            nodes_accounted_for.append(tuple[0])
            clean_open.append(tuple)
        else: # Si el nodo ya está metido en la lista, solo lo ignoramos ya que el que se encuentra tiene el mismo peso.
            pass
...
greedy(get_node(node_list, start_node.upper()), get_node(node_list, goal_node.upper()))