...
# =========================================================
# Implementaciones relacionadas al algoritmo A estrella
# =========================================================

def a_star(start_node, goal_node):
    current = (None, None, None, None, None) # Tupla encargada de llevar registro del nodo actual (1), nodo desde el que se llegó (2), el costo (3), el valor heurístico (4) y la suma de estos 2(5)
    open = [] # Lista de tuplas encargada de llevar registro de la lista Abiertos. Nodo (1), nodo desde el que se llegó (2), el costo (3), el valor heurístico (4) y la suma de estos 2(5)
    closed = [] # Lista de tuplas encargada de llevar registro de la lista Cerrados. Nodo (1), nodo desde el que se llegó (2), el costo (3), el valor heurístico (4) y la suma de estos 2(5)
    path = []

    print()
    print("Actual: -")
    print(f"Abiertos: [{start_node.name}_{start_node.name}: {straight_line_distance[start_node.name, start_node.name]}: {straight_line_distance[start_node.name, goal_node.name]}: {straight_line_distance[start_node.name, start_node.name]+straight_line_distance[start_node.name, goal_node.name]}]")
    print("Cerrados: []")

    new_tuple = (
        start_node,
        start_node,
        straight_line_distance[start_node.name, start_node.name],
        straight_line_distance[start_node.name, goal_node.name],
        straight_line_distance[start_node.name, start_node.name]+straight_line_distance[start_node.name, goal_node.name]
    )
    open.append(new_tuple)

    while current[0] is not goal_node:
        open_lowest = open.pop(get_lowest_tuple_index(open))
        current = (open_lowest[0], open_lowest[1], open_lowest[2], open_lowest[3], open_lowest[4])
        ...
        for neighbor in current[0].neighbors:
            ...
            if not node_is_in_closed(neighbor, closed):
                new_tuple = (
                    neighbor,
                    current[0],
                    straight_line_distance[neighbor.name, current[0].name]+current[2],
                    straight_line_distance[neighbor.name, goal_node.name],
                    straight_line_distance[neighbor.name, current[0].name]+current[2]+straight_line_distance[neighbor.name, goal_node.name]
                )
                open.append(new_tuple)
...
def print_step(current, open, closed):
    # Impimir actual
    print()
    print(f"Actual: {current[0].name}_{current[1].name}: {current[2]}: {current[3]}: {current[4]}")

    # Imprimir lista Abiertos
    print(f"Abiertos: [", end="")
    for tuple in open:
        print(f"{tuple[0].name}_{tuple[1].name}: {tuple[2]}: {tuple[3]}: {tuple[4]}", end=", ")
    print(f"]")
...
a_star(get_node(node_list, start_node.upper()), get_node(node_list, goal_node.upper()))